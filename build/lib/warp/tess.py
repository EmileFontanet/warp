...
def detrend(time, flux, flux_err=None,
    ...
    med = np.nanmedian(f)
    f = f / med
    ferr = ferr / med if ferr is not None else None
    # sigma clipping
    ...
    if (goal == 'astero'):
        ...
        t = t[good]
        f = f[good]
        ferr = ferr[good] if ferr is not None else None
        ...
        flux_flat = f / trend
        err_flat = ferr / trend if ferr is not None else None
        if return_trend:
            return t, flux_flat, trend, err_flat

        return t, flux_flat, err_flat
    elif (goal == 'transit'):
