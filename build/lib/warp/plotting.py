...
def plot_lightcurve(
    ...
    from .tess import detrend

    hdul = fits.open(lc_file)
    lc_data = hdul[1].data
    time = lc_data['TIME']
    quality = lc_data['QUALITY']
    if 'PDCSAP_FLUX' in lc_data.columns.names:
        flux_origin = 'PDCSAP_FLUX'
    elif 'KSPSAP_FLUX' in lc_data.columns.names:
        flux_origin = 'KSPSAP_FLUX'
    elif 'SAP_FLUX' in lc_data.columns.names:
        flux_origin = 'SAP_FLUX'
    else:
        raise ValueError("No usable flux column found")
    flux = lc_data[flux_origin]
    flux_err = lc_data[flux_origin + '_ERR'] if flux_origin + \
        '_ERR' in lc_data.columns.names else None
    sector = hdul[0].header.get("SECTOR", None)
    hdul.close()

    if not show_bad_points:
        mask = quality == 0
        time = time[mask]
        flux = flux[mask]
        flux_err = flux_err[mask] if flux_err is not None else None
    if detrend:
        t_lc, flux_flat, trend_full, err_flat = detrend(time, flux, flux_err=flux_err,
        ...
        flux = flux/med
        flux_err = flux_err / \
            med if flux_err is not None else None
    if ax is None:
        if show_raw and detrend:
            fig, (ax1, ax2) = plt.subplots(
                2, 1, figsize=kwargs.get('figsize', (10, 6)), sharex=True)
            ax1.set_title(
                f"{star_name} - Sector {sector}" if sector is not None else f"{star_name}")
            ax1.errorbar(time, flux, yerr=flux_err, fmt='.',
                         alpha=0.5, label='Raw flux', **kwargs)
            ax1.plot(t_lc, trend_full, color='tab:orange',
                     label='Trend', **kwargs)
            ax1.set_xlabel('Time [d]')
            ax1.set_ylabel(flux_origin)
            ax1.legend()
            ax2.set_title(
                f"{star_name} - Detrended flux" if sector is not None else f"{star_name}")
            ax2.errorbar(t_lc, flux_flat, yerr=err_flat, fmt='.',
                         alpha=0.5, label='Detrended flux', **kwargs)
