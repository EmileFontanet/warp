...
def process_lc_file(lc_path,  guess_table=None, plot=False, min_nu=None, max_nu=None):
    try:
        ...
        time, flux, trend_full, _ = detrend(
            time, flux, flux_err, return_trend=True, smooth_days=10)
        ...
        if isinstance(guess_table, float) or isinstance(guess_table, int):
            guess_nu = guess_table
        elif guess_table is not None:
        ...
        return {
            ...
            'flux': flux_col,
            "freq": results.get("freq", np.nan),
            "power": results.get("power", np.nan),
        }
