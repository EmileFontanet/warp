...
# This file contains the nuSYD code, developped by K. R Sreenivas
# The documentation and references ca be found on its repo: https://github.com/redeaglekr/nuSYD
# nuSYD utilizes the methodology presented in Sreenivas et al 2024 (https://academic.oup.com/mnras/article/530/3/3477/7643660?login=true)
...
class nuSYD:
    ...
    def _get_numax_frompds(self, freq, power, lc_type, nyq):
        if self.lc_type == "TESS":
            ...
            # guess_numax = min(guess_numax, 400)
            # guess_numax = max(guess_numax, 15)
    ...
    def _refine_numax(self, freq, power, guess_numax, nyq):
        ...
        numax_actual = (numax_uncor_final**2 - 2 *
                        (sigma_actual)**2)/numax_uncor_final
    ...
    def run(self):
        ...
        # dt_sec = np.nanmedian(np.diff(self.time))*60*60*24
        # nyq = 1 / (2 * dt_sec)
        # self.nyq = nyq
        time_eff = self._eff(self.time) if self.lc_type == "TESS" else len(
            self.time)*np.median(np.diff(self.time))
        ...
        if self.guess_numax == "from_lc":
            guess_numax = self._get_numax_frompds(
                freq, power, self.lc_type,   nyq)
            self.guess_numax = guess_numax

        # core nuSYD
        numax, numax_uncor, ppeak, width_l, width_r, smoothed_power, divided_power = self._refine_numax(
            freq, power, self.guess_numax, nyq)
    ...
    def _plot_results(self):
        ...
        max_power = np.max(self.power[(
            self.freq > 0.6*self.numax_uncor) & (self.freq < 1.5*self.numax_uncor)])
        pf = max_power/self.ppeak  # plotting factor to enlarge the smoothed power

        axs[0].plot(self.freq, self.power, c="tab:blue")
        axs[0].axvline(self.guess_numax, color="blue", ls="dashed",
                       label=r"Initial guess for $\nu_{max}$ = "+str(round(self.guess_numax, 4))+r"$\rm \mu$Hz")
        axs[0].legend()
        ...
        axs[1].axvline(self.numax_uncor, color="tab:red", linestyle="--",
                       label=r"$\nu_{\mathrm{max}}$ (uncorrected) = "+str(round(self.numax_uncor, 4))+r" $\rm \mu$Hz")
        axs[1].axvline(self.numax, color="black", linestyle="--",
                       label=r"$\nu_{\mathrm{max}}$ (final) = "+str(round(self.numax, 4))+r" $\rm \mu$ Hz")
        axs[1].scatter((self.widths[0], self.widths[1]),
                       (pf*self.ppeak/2, pf*self.ppeak/2), color="tab:green")
        axs[1].plot((self.widths[0], self.widths[1]), (pf*self.ppeak/2, pf*self.ppeak/2), color="tab:green",
                    ls="dotted", label=r"Width = {:.4f} $\rm \mu$Hz".format(self.widths[1]-self.widths[0]))
    ...
    def montecarlo(self, freq, power, numax, nyq):
        ...
        def _single_mc(_):
            ...
            if self.lc_type == "Kepler":
                wn_l, wn_h = (0.97 * nyq, nyq) if numax < 0.9 * \
                    nyq else (0.68 * numax, 0.72 * numax)
            else:
                wn_l, wn_h = (0.91 * nyq, nyq) if numax < 0.8 * \
                    nyq else (0.68 * numax, 0.72 * numax)
        ...
        results = Parallel(n_jobs=5, backend='threading')(
            delayed(_single_mc)(i) for i in range(self.mc_iter))
